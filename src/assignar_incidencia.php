...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // Assignar el tècnic (empleat) a la incidència
...
<form method="POST">
<h1>Assignar Tècnic a la Incidència <?= htmlspecialchars($id) ?></h1>

    <label for="empleat">Selecciona un tècnic:</label>
    <select name="empleat" id="empleat">
        <option value="">Selecciona un tècnic</option>
        <?php while ($row = $empleats->fetch_assoc()): ?>
            <option value="<?= $row['DNI'] ?>"><?= $row['Nom'] ?></option>
        <?php endwhile; ?>
    </select>
    <br><br>
